...
```{r}
occurrence %<>% mutate(rightsHolder = "UVW") # e.g. "INBO"
```
### accessRights
...
```{r}
occurrence %<>% mutate(datasetName = "Mica UVW Muskrat Occurrences in The Netherlands") # e.g. "Checklist of non-native freshwater fishes in Flanders, Belgium"
```
...
#                mutate(occurrenceID = str_c("UVW:RATO:", occurrenceID))
...
```{r}
occurrence %<>% mutate(recordedBy = 'NLwaterschappen')
```
...
```{r}
occurrence %<>% rename(individualCount='ï..NumberOfCatches') 
```
...
occurrence %<>% rename(decimalLongitude = X) %>%
                ...
                mutate(verbatimCoordinateSystem = "Rijksdriehoeknet") %>%
                mutate(verbatimSRS = "EPSG:28993") %>%
...
```{r}
#occurrence %<>% mutate(verbatimcoordinatesystem = "Rijksdriehoeknet Amersfoort/RD new") # 
```
